...
public class UiV2Group {
    ...
    try {
      ...
      if (!GrouperRequestContainer.retrieveFromRequestOrCreate().getGroupContainer().isCanReadPrivilegeInheritance()) {
        throw new RuntimeException(
            "Not allowed to read privilege inheritance! " + GrouperUtil.subjectToString(loggedInSubject));
      }
  ...
  public void viewGroup(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      guiResponseJs.addAction(
          GuiScreenAction.newInnerHtmlFromJsp("#grouperMainContentDivId", "/WEB-INF/grouperUi2/group/viewGroup.jsp"));
  ...
  /**
   * the filter button was pressed, or paging or sorting, or view Group or
   * something
   * 
   * @param request
   * @param response
  ...
  public void removeMemberForThisGroupsMemberships(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      GrouperUserDataApi.recentlyUsedGroupAdd(GrouperUiUserData.grouperUiGroupNameForUserData(), loggedInSubject,
          group);
  ...
  public void removeMember(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      GrouperUserDataApi.recentlyUsedMemberAdd(GrouperUiUserData.grouperUiGroupNameForUserData(), loggedInSubject,
          member);
  ...
  private void filterHelper(HttpServletRequest request, HttpServletResponse response, Group group) {
    ...
    MembershipFinder membershipFinder = new MembershipFinder().addGroupId(group.getId()).assignCheckSecurity(true)
        .assignHasFieldForMember(true).assignEnabled(true).assignHasMembershipTypeForMember(true)
        .assignQueryOptionsForMember(queryOptions).assignSplitScopeForMember(true);
    ...
    Set<MembershipSubjectContainer> results = membershipFinder.findMembershipResult().getMembershipSubjectContainers();
    ...
    guiResponseJs.addAction(
        GuiScreenAction.newInnerHtmlFromJsp("#groupFilterResultsId", "/WEB-INF/grouperUi2/group/groupContents.jsp"));
  ...
  public void addMemberFilter(HttpServletRequest request, HttpServletResponse response) {
    ...
    DojoComboLogic.logic(request, response, new DojoComboQueryLogicBase<Subject>() {
      ...
      public Subject lookup(HttpServletRequest localRequest, GrouperSession grouperSession, String query) {
        ...
        // when we refer to subjects in the dropdown, we will use a sourceId / subject
        // tuple
          ...
          if (query != null && query.contains("||")) {
            ...
            try {
              ...
              final String requireSources = GrouperUiConfig.retrieveConfig()
                  .propertyValueString("uiV2.subjectLookupRequireSources");
      ...
      public Collection<Subject> search(HttpServletRequest localRequest, GrouperSession grouperSession, String query) {
        ...
        try {
          ...
          final String requireSources = GrouperUiConfig.retrieveConfig()
              .propertyValueString("uiV2.subjectSearchRequireSources");

          Collection<Subject> results = null;
          if (!StringUtils.isBlank(requireSources)) {
            Set<Source> sources = GrouperUtil.convertSources(requireSources);
            results = StringUtils.isBlank(stemName) ? SubjectFinder.findPage(query, sources).getResults()
                : SubjectFinder.findPageInStem(stemName, query, sources).getResults();

          } else {
            results = StringUtils.isBlank(stemName) ? SubjectFinder.findPage(query).getResults()
      ...
      public String initialValidationError(HttpServletRequest localRequest, GrouperSession grouperSession) {
        ...
        // MCH 20140316
        // Group group = retrieveGroupHelper(request,
        // AccessPrivilege.UPDATE).getGroup();
        //
        // if (group == null) {
        //
  ...
  public void addMemberSearch(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      if (group != null) {
        ...
        if (stem != null) {
          ...
          AttributeDef attributeDef = UiV2AttributeDef
              .retrieveAttributeDefHelper(request, AttributeDefPrivilege.ATTR_ADMIN, true).getAttributeDef();
      ...
      if (matchExactId) {
        if (GrouperConfig.retrieveConfig().propertyValueBoolean("grouperQuerySubjectsMultipleQueriesCommaSeparated",
            true)) {
          ...
          if (StringUtils.equals("all", sourceId)) {
            subjects = new LinkedHashSet<Subject>(
                GrouperUtil.nonNull(SubjectFinder.findByIdsOrIdentifiers(searchStrings)).values());
          } else {
            subjects = new LinkedHashSet<Subject>(
                GrouperUtil.nonNull(SubjectFinder.findByIdsOrIdentifiers(searchStrings, sourceId)).values());
          }
        } else {
          Subject subject = null;
          if (StringUtils.equals("all", sourceId)) {
      ...
      guiResponseJs.addAction(
          GuiScreenAction.newInnerHtmlFromJsp("#addMemberResults", "/WEB-INF/grouperUi2/group/addMemberResults.jsp"));
   ...
   * @return
   */
  public boolean callOccursCheckBoolean(final String parentId, final String childId) throws GrouperDAOException {
    boolean result = false;
    try {
      result = (Boolean) HibernateSession.callbackHibernateSession(GrouperTransactionType.READONLY_OR_USE_EXISTING,
          AuditControl.WILL_NOT_AUDIT, new HibernateHandler() {
            ...
            public Object callback(HibernateHandlerBean hibernateHandlerBean) throws GrouperDAOException {
              ...
              try {
                ...
                // "SELECT group_cycle_occurs_check(:parent_group_id, :child_group_id) FROM
                // DUAL"); // session.getNamedQuery("groupCycleOccursCheck")
                // .setParameter("parentId", parentId).setParameter("childId", childId);

                SQLQuery statement = session
                    .createSQLQuery("SELECT group_cycle_occurs_check(:parent_group_id, :child_group_id) FROM DUAL");
    ...
    try {
...

      {
        ...
        if (StringUtils.equals(privilegeOptionsValue, "default")) {
          ...
          throw new RuntimeException(
              "For privilege-options expecting default or custom but was: '" + privilegeOptionsValue + "'");
        }
      }
      ...
      boolean attrUpdateChecked = GrouperUtil.booleanValue(request.getParameter("privileges_groupAttrUpdaters[]"),
          false);

      if (!defaultPrivs && !memberChecked && !adminChecked && !updateChecked && !readChecked && !viewChecked
          && !optinChecked && !optoutChecked && !attrReadChecked && !attrUpdateChecked) {
        guiResponseJs.addAction(GuiScreenAction.newValidationMessage(GuiMessageType.error, "#groupPrivsErrorId",
      ...
      if (null != subject.getId() && 32 == subject.getId().length()
        ...
        guiResponseJs.addAction(GuiScreenAction.newMessage(GuiMessageType.error,
            TextContainer.retrieveFromRequest().getText().get("groupAddMemberWouldCreateCycle")));
        return;
      }

      boolean madeChanges = false;
      try {
        madeChanges = group.addOrEditMember(subject, defaultPrivs, memberChecked, adminChecked, updateChecked,
      ...
      // clear out the combo
      guiResponseJs
          .addAction(GuiScreenAction.newScript("dijit.byId('groupAddMemberComboId').set('displayedValue', ''); "
  ...
  /**
   * assign or remove a privilege from a user, and redraw the filter screen... put
   * a success at top
   * 
   * @param request
   * @param response
...
//    }
  ...
  /**
   * submit the main form on the privilege screen which can do batch operations on
   * a number of rows
   * 
   * @param request
   * @param response
  ...
  public void assignPrivilegeBatch(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      if (!operationMatcher.matches()) {
        throw new RuntimeException(
            "Invalid submission, should have a valid operation: '" + groupPrivilegeBatchUpdateOperation + "'");
      }
      ...
      if (assignAll) {
        if (assign) {
          ...
          privileges = new Privilege[] { AccessPrivilege.listToPriv(Field.FIELD_NAME_GROUP_ATTR_READERS),
      ...
      if (changes > 0) {
        guiResponseJs.addAction(GuiScreenAction.newMessage(GuiMessageType.success, TextContainer.retrieveFromRequest()
            .getText().get(assign ? "groupSuccessGrantedPrivileges" : "groupSuccessRevokedPrivileges")));
      } else {
        guiResponseJs.addAction(GuiScreenAction.newMessage(GuiMessageType.info, TextContainer.retrieveFromRequest()
            .getText().get(assign ? "groupNoteNoGrantedPrivileges" : "groupNoteNoRevokedPrivileges")));
  ...
  /**
   * the filter button for privileges was pressed, or paging or sorting or
   * something
   * 
   * @param request
   * @param response
  ...
  /**
   * the filter button for this groups memberships was pressed, or paging or
  ...
  /**
   * the filter button was pressed for privileges, or paging or sorting, or view
   * Group privileges or something
   * 
   * @param request
   * @param response
   * @param group
  ...
  private void filterPrivilegesHelper(HttpServletRequest request, HttpServletResponse response, Group group) {
    ...
    MembershipFinder membershipFinder = new MembershipFinder().addGroupId(group.getId()).assignCheckSecurity(true)
        .assignFieldType(FieldType.ACCESS).assignEnabled(true).assignHasFieldForMember(true)
        .assignHasMembershipTypeForMember(true).assignQueryOptionsForMember(queryOptions)
    ...
    try {
      ...
      if (GrouperUiConfig.retrieveConfig().propertyValueBoolean("uiV2.group.checkForFactorWhenDeletingGroup", true)) {
        ...
        if (GrouperUtil.length(composites) > 0) {
          ...
          result.append(TextContainer.retrieveFromRequest().getText().get("groupProblemDeleteWithCompositeFactor"))
              .append(" ");
      ...
      guiResponseJs.addAction(
          GuiScreenAction.newInnerHtmlFromJsp("#grouperMainContentDivId", "/WEB-INF/grouperUi2/group/groupDelete.jsp"));
  ...
  public void groupDeleteSubmit(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      try {
        ...
        guiResponseJs.addAction(
            GuiScreenAction.newScript("guiV2link('operation=UiV2Group.viewGroup&groupId=" + group.getId() + "')"));
      ...
      guiResponseJs
          .addAction(GuiScreenAction.newScript("guiV2link('operation=UiV2Stem.viewStem&stemId=" + stemId + "')"));

      // lets show a success message on the new screen
      guiResponseJs.addAction(GuiScreenAction.newMessage(GuiMessageType.success,
          TextContainer.retrieveFromRequest().getText().get("groupDeleteSuccess")));

      GrouperUserDataApi.recentlyUsedGroupRemove(GrouperUiUserData.grouperUiGroupNameForUserData(), loggedInSubject,
          group);
  ...
  public void newGroupSubmit(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      final Stem parentFolder = new StemFinder().assignPrivileges(NamingPrivilege.CREATE_PRIVILEGES)
          .assignSubject(loggedInSubject).assignScope(parentFolderId).assignFindByUuidOrName(true).findStem();
      ...
      if (StringUtils.isBlank(displayExtension)) {
        ...
        guiResponseJs.addAction(GuiScreenAction.newValidationMessage(GuiMessageType.error, "#groupName",
      ...
      if (StringUtils.isBlank(extension)) {
        ...
        guiResponseJs.addAction(GuiScreenAction.newValidationMessage(GuiMessageType.error, "#groupId",
      ...
      group = (Group) GrouperSession.callbackGrouperSession(grouperSession.internal_getRootSession(),
   ...
   * @param gve
   */
  private void handleGrouperValidationException(GuiResponseJs guiResponseJs, GrouperValidationException gve) {
    // groupValidation_groupDescriptionTooLong = Error, group description is too
    // long
    ...
    if (StringUtils.equals(Group.VALIDATION_GROUP_DESCRIPTION_TOO_LONG_KEY, gve.getGrouperValidationKey())) {
      ...
      guiResponseJs.addAction(GuiScreenAction.newValidationMessage(GuiMessageType.error, "#groupDescription",
  ...
  public void newGroup(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      guiResponseJs.addAction(
          GuiScreenAction.newInnerHtmlFromJsp("#grouperMainContentDivId", "/WEB-INF/grouperUi2/group/newGroup.jsp"));
  ...
  public void groupEdit(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      GrouperRequestContainer.retrieveFromRequestOrCreate().getGroupContainer().getGuiGroup()
          .setShowBreadcrumbLink(true);
      GrouperRequestContainer.retrieveFromRequestOrCreate().getGroupContainer().getGuiGroup()
          .setShowBreadcrumbLinkSeparator(false);

      GuiResponseJs guiResponseJs = GuiResponseJs.retrieveGuiResponseJs();

      guiResponseJs.addAction(
          GuiScreenAction.newInnerHtmlFromJsp("#grouperMainContentDivId", "/WEB-INF/grouperUi2/group/groupEdit.jsp"));
  ...
  public void groupEditSubmit(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      try {
        ...
        // create the group
        GroupSave groupSave = new GroupSave(GROUPER_SESSION).assignUuid(group.getId()).assignSaveMode(SaveMode.UPDATE)
            .assignName(group.getParentStemName() + ":" + extension).assignDisplayExtension(displayExtension)
            .assignDescription(description).assignTypeOfGroup(typeOfGroup).assignPrivAllAdmin(adminChecked)
            .assignPrivAllAttrRead(attrReadChecked).assignPrivAllAttrUpdate(attrUpdateChecked)
...
//
  ...
  public static RetrieveGroupHelperResult retrieveGroupHelper(HttpServletRequest request, Privilege requirePrivilege,
    ...
    if (group != null) {
      ...
      if (privsOk) {
        ...
        List<GrouperObjectTypesAttributeValue> attributeValuesForGroup = GrouperObjectTypesConfiguration
            .getGrouperObjectTypesAttributeValues(group);
        grouperRequestContainer.getObjectTypeContainer().setGuiConfiguredGrouperObjectTypesAttributeValues(
            GuiGrouperObjectTypesAttributeValue.convertFromGrouperObjectTypesAttributeValues(attributeValuesForGroup));

        GrouperLoaderContainer grouperLoaderContainer = GrouperRequestContainer.retrieveFromRequestOrCreate()
      ...
      if (!addedError
          && (!StringUtils.isBlank(groupId) || !StringUtils.isBlank(groupName) || !StringUtils.isBlank(groupIndex))) {
    ...
    if (addedError && errorIfNotFound) {
      guiResponseJs.addAction(
          GuiScreenAction.newInnerHtmlFromJsp("#grouperMainContentDivId", "/WEB-INF/grouperUi2/index/indexMain.jsp"));
    }
  ...
  /**
   * the filter button was pressed of this groups memberships screen, or paging or
   * sorting, or view Group or something
   * 
   * @param request
   * @param response
   * @param group
   */
  private void filterThisGroupsMembershipsHelper(HttpServletRequest request, HttpServletResponse response,
      Group group) {
    ...
    MembershipFinder membershipFinder = new MembershipFinder().addMemberId(group.toMember().getId())
        .assignCheckSecurity(true).assignHasFieldForGroup(true).assignEnabled(true)
        .assignHasMembershipTypeForGroup(true).assignQueryOptionsForGroup(queryOptions).assignSplitScopeForGroup(true);
  ...
  public void removeMembers(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      // subject has update, so this operation as root in case removing affects the
      // membership
  ...
  /**
   * the filter button for this groups privileges was pressed, or paging or
   * sorting or something
   * 
   * @param request
   * @param response
  ...
  /**
   * the filter button was pressed of this groups group privileges screen, or
   * paging or sorting, or view Group or something
   * 
   * @param request
   * @param response
   * @param group
   */
  private void filterThisGroupsGroupPrivilegesHelper(HttpServletRequest request, HttpServletResponse response,
      Group group) {
    ...
    MembershipFinder membershipFinder = new MembershipFinder().addSubject(group.toSubject()).assignCheckSecurity(true)
        .assignFieldType(FieldType.ACCESS).assignEnabled(true).assignHasFieldForGroup(true)
        .assignHasMembershipTypeForGroup(true).assignQueryOptionsForGroup(queryOptions).assignSplitScopeForGroup(true);
  ...
  public void thisGroupsPrivilegesAssignPrivilege(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      if (parentGroup == null
          || !parentGroup.canHavePrivilege(loggedInSubject, AccessPrivilege.ADMIN.getName(), false)) {
        guiResponseJs.addAction(GuiScreenAction.newMessage(GuiMessageType.success,
  ...
  /**
   * submit the main form on the this groups privilege screen which can do batch
   * operations on a number of rows
   * 
   * @param request
   * @param response
   */
  public void thisGroupsPrivilegesAssignPrivilegeBatch(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      Privilege[] privileges = assignAll
          ? (assign ? new Privilege[] { AccessPrivilege.listToPriv(Field.FIELD_NAME_ADMINS) }
              : new Privilege[] { AccessPrivilege.listToPriv(Field.FIELD_NAME_ADMINS),
  ...
  /**
   * the filter button for this groups stem privileges was pressed, or paging or
   * sorting or something
   * 
   * @param request
   * @param response
  ...
  /**
   * the filter button was pressed of this groups stem privileges screen, or
   * paging or sorting, or view Group or something
   * 
   * @param request
   * @param response
   * @param group
   */
  private void filterThisGroupsStemPrivilegesHelper(HttpServletRequest request, HttpServletResponse response,
      Group group) {
    ...
    MembershipFinder membershipFinder = new MembershipFinder().addSubject(group.toSubject()).assignCheckSecurity(true)
        .assignFieldType(FieldType.NAMING).assignEnabled(true).assignHasFieldForStem(true)
        .assignHasMembershipTypeForStem(true).assignQueryOptionsForStem(queryOptions).assignSplitScopeForStem(true);
  ...
  public void thisGroupsPrivilegesAssignStemPrivilege(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      if (parentStem == null
          || !parentStem.canHavePrivilege(loggedInSubject, NamingPrivilege.STEM_ADMIN.getName(), false)) {
        guiResponseJs.addAction(GuiScreenAction.newMessage(GuiMessageType.success,
  ...
  /**
   * submit the main form on the this groups stem privilege screen which can do
   * batch operations on a number of rows
   * 
   * @param request
   * @param response
   */
  public void thisGroupsPrivilegesAssignStemPrivilegeBatch(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      Privilege[] privileges = assignAll
          ? (assign ? new Privilege[] { NamingPrivilege.listToPriv(Field.FIELD_NAME_STEM_ADMINS) }
              : new Privilege[] { NamingPrivilege.listToPriv(Field.FIELD_NAME_STEM_ADMINS),
  ...
  /**
   * the filter button for this groups attribute def privileges was pressed, or
   ...
   * @param group
   */
  private void filterThisGroupsAttributeDefPrivilegesHelper(HttpServletRequest request, HttpServletResponse response,
      Group group) {
    ...
    MembershipFinder membershipFinder = new MembershipFinder().addSubject(group.toSubject()).assignCheckSecurity(true)
        .assignFieldType(FieldType.ATTRIBUTE_DEF).assignEnabled(true).assignHasFieldForAttributeDef(true)
        .assignHasMembershipTypeForAttributeDef(true).assignQueryOptionsForAttributeDef(queryOptions)
   ...
   * @param response
   */
  public void thisGroupsPrivilegesAssignAttributeDefPrivilegeBatch(HttpServletRequest request,
      HttpServletResponse response) {
    ...
    try {
      ...
      Privilege[] privileges = assignAll
          ? (assign ? new Privilege[] { AttributeDefPrivilege.listToPriv(Field.FIELD_NAME_ATTR_ADMINS) }
              : new Privilege[] { AttributeDefPrivilege.listToPriv(Field.FIELD_NAME_ATTR_ADMINS),
          ...
          : (readersUpdaters
              ? new Privilege[] { AttributeDefPrivilege.listToPriv(Field.FIELD_NAME_ATTR_READERS),
                  AttributeDefPrivilege.listToPriv(Field.FIELD_NAME_ATTR_UPDATERS) }
      ...
      for (AttributeDef parentAttributeDef : parentAttributeDefs) {
        ...
        for (Privilege privilege : privileges) {
          if (assign) {
            ...
            changes += parentAttributeDef.getPrivilegeDelegate().revokePriv(group.toSubject(), privilege, false) ? 1
                : 0;
          }
        }
      }
  ...
  /**
   * combo filter create group folder. Note, this cannot be composite (since you
   * cant add members to a composite)
   * 
   * @param request
   * @param response
   */
    ...
    DojoComboLogic.logic(request, response, new DojoComboQueryLogicBase<Group>() {
      ...
      public Group lookup(HttpServletRequest localRequest, GrouperSession grouperSession, String query) {
        ...
        GroupFinder groupFinder = new GroupFinder().assignPrivileges(AccessPrivilege.UPDATE_PRIVILEGES)
            .assignSubject(loggedInSubject).assignCompositeOwner(false).assignFindByUuidOrName(true).assignScope(query);
      ...
      public Collection<Group> search(HttpServletRequest localRequest, GrouperSession grouperSession, String query) {
        ...
        GroupFinder groupFinder = new GroupFinder().assignPrivileges(AccessPrivilege.UPDATE_PRIVILEGES)
            .assignScope(query).assignSubject(loggedInSubject).assignCompositeOwner(false).assignSplitScope(true)
  ...
  /**
   * the audit filter button was pressed, or paging or sorting, or view audits or
   * something
   * 
   * @param request
   * @param response
  ...
  private void viewAuditsHelper(HttpServletRequest request, HttpServletResponse response, Group group) {
    ...
    if (StringUtils.equals(filterTypeString, "on") || StringUtils.equals(filterTypeString, "before")
      ...
      if (StringUtils.isBlank(filterFromDateString)) {
        guiResponseJs.addAction(GuiScreenAction.newValidationMessage(GuiMessageType.error, "#from-date",
    ...
    if (StringUtils.equals(filterTypeString, "between")) {
      if (StringUtils.isBlank(filterToDateString)) {
        guiResponseJs.addAction(GuiScreenAction.newValidationMessage(GuiMessageType.error, "#to-date",
...
//    }
    ...
    DojoComboLogic.logic(request, response, new DojoComboQueryLogicBase<Group>() {
      ...
      public Group lookup(HttpServletRequest localRequest, GrouperSession grouperSession, String query) {
        ...
        Group theGroup = new GroupFinder().assignPrivileges(AccessPrivilege.READ_PRIVILEGES)
            .assignSubject(loggedInSubject).assignFindByUuidOrName(true).assignScope(query).findGroup();
        return theGroup;
      }
      ...
      public Collection<Group> search(HttpServletRequest localRequest, GrouperSession grouperSession, String query) {
        ...
        int groupComboSize = GrouperUiConfig.retrieveConfig().propertyValueInt("uiV2.groupComboboxResultSize", 200);
        QueryOptions queryOptions = QueryOptions.create(null, null, 1, groupComboSize);
        return new GroupFinder().assignPrivileges(AccessPrivilege.READ_PRIVILEGES).assignScope(query)
            .assignSubject(loggedInSubject).assignSplitScope(true).assignQueryOptions(queryOptions).findGroups();
      }
  ...
  public void groupEditCompositeSubmit(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      Group leftFactorGroup = StringUtils.isBlank(leftFactorGroupId) ? null
          : new GroupFinder().assignScope(leftFactorGroupId).assignPrivileges(AccessPrivilege.READ_PRIVILEGES)
              .assignFindByUuidOrName(true).assignSubject(loggedInSubject).findGroup();

      if (leftFactorGroup == null) {
        guiResponseJs.addAction(
            GuiScreenAction.newValidationMessage(GuiMessageType.error, "#groupCompositeLeftFactorComboErrorId",
      ...
      // we dont allow new groups to have union, or switch to union. shouldnt be
      // possible unles someone is hacking...
      ...
      if (compositeType == null) {
        guiResponseJs.addAction(GuiScreenAction.newValidationMessage(GuiMessageType.error, "#compositeOperationId",
      ...
      Group rightFactorGroup = StringUtils.isBlank(rightFactorGroupId) ? null
          : new GroupFinder().assignScope(rightFactorGroupId).assignPrivileges(AccessPrivilege.READ_PRIVILEGES)
              .assignFindByUuidOrName(true).assignSubject(loggedInSubject).findGroup();

      if (rightFactorGroup == null) {
        guiResponseJs.addAction(
            GuiScreenAction.newValidationMessage(GuiMessageType.error, "#groupCompositeRightFactorComboErrorId",
  ...
  /**
   * force grouperLoader to update this loader group don't throw exception,
   * display success or error message directly on New Ui screen
   * 
   * @param request
   * @param response
   */
  public void updateLoaderGroup(HttpServletRequest request, HttpServletResponse response) {
    ...
    if (!GrouperUiConfig.retrieveConfig().propertyValueBoolean("uiV2.group.allowGroupAdminsToRefreshLoaderJobs",
        true)) {
      throw new RuntimeException(
  ...
  public void viewAllLoaderManagedGroups(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      AttributeDefName loaderMetadataLoadedAttributeDefName = AttributeDefNameFinder.findByName(
          GrouperCheckConfig.loaderMetadataStemName() + ":" + GrouperLoader.ATTRIBUTE_GROUPER_LOADER_METADATA_LOADED,
          false);

      // get all groups with settings
      int maxPageSize = GrouperUiConfig.retrieveConfig().propertyValueInt("grouperUi.grouperLoader.maxGroupsShown",
          200);
      Set<Group> groupsWithLoaderMetadata = new GroupFinder().assignPrivileges(AccessPrivilege.VIEW_PRIVILEGES)
          ...
          .assignQueryOptions(QueryOptions.create("displayName", true, 1, maxPageSize)).findGroups();
      ...
      if (GrouperUtil.length(groupsWithLoaderMetadata) > 0) {
        ...
        AttributeAssignValueFinderResult attributeAssignValueFinderResult = new AttributeAssignValueFinder()
            .assignOwnerGroupsOfAssignAssign(groupsWithLoaderMetadata).addAttributeDefNameId(attributeDefName.getId())
            .assignAttributeCheckReadOnAttributeDef(false).findAttributeAssignValuesResult();

        guiLoaderManagedGroups.addAll(GuiLoaderManagedGroup
            .convertGroupIntoGuiLoaderManagedGroups(groupsWithLoaderMetadata, attributeAssignValueFinderResult));
  ...
  /**
   * schedule loader job don't throw exception, display success or error message
   * directly on New Ui screen
   * 
   * @param request
   * @param response
   */
  public void scheduleLoaderGroup(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      try {
        ...
        // check sql first
        AttributeDefName grouperLoader = GrouperDAOFactory.getFactory().getAttributeDefName().findByNameSecure(
            GrouperConfig.retrieveConfig().propertyValueString("grouper.rootStemForBuiltinObjects", "etc")
                + ":legacy:attribute:legacyGroupType_grouperLoader",
            false);
        ...
        if (!foundLoaderType) {
          ...
          if (grouperLoaderLdapName != null) {
            AttributeAssign assign = group.getAttributeDelegate().retrieveAssignment("assign", grouperLoaderLdapName,
                true, false);
            if (assign != null) {
              foundLoaderType = true;
        ...
        // lets show an error message on the new screen
        guiResponseJs.addAction(GuiScreenAction.newMessage(GuiMessageType.error,
            TextContainer.retrieveFromRequest().getText().get("loaderGroupScheduleError") + "<br />" + e.getMessage()));
        return;
      }
   ...
   * @param response
   */
  public void inheritedPrivilegesAssignedToThisGroupFromFolders(HttpServletRequest request,
      HttpServletResponse response) {
  ...
  public void convertGroupToRole(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      guiResponseJs.addAction(GuiScreenAction
          .newScript("guiV2link('operation=UiV2GroupPermission.groupPermission&groupId=" + group.getId() + "')"));
    ...
    DojoComboLogic.logic(request, response, new DojoComboQueryLogicBase<Group>() {
      ...
      public Group lookup(HttpServletRequest localRequest, GrouperSession grouperSession, String query) {
        ...
        GroupFinder groupFinder = new GroupFinder().assignPrivileges(AccessPrivilege.ATTRIBUTE_UPDATE_PRIVILEGES)
            .assignSubject(loggedInSubject).assignFindByUuidOrName(true).assignScope(query)
      ...
      public Collection<Group> search(HttpServletRequest localRequest, GrouperSession grouperSession, String query) {
        ...
        QueryOptions queryOptions = QueryOptions.create(null, null, 1, groupComboSize);
        GroupFinder groupFinder = new GroupFinder().assignPrivileges(AccessPrivilege.ATTRIBUTE_UPDATE_PRIVILEGES)
            .assignScope(query).assignSubject(loggedInSubject).assignSplitScope(true).assignQueryOptions(queryOptions)
            .addTypeOfGroup(TypeOfGroup.role);
